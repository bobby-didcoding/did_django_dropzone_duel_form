...
def images(request):
	...
	i_form = ImageCommentForm()
	result = "error"
	message = "Something went wrong. Please check and try again"

	if request.is_ajax() and request.method == "POST":
		i_form = ImageCommentForm(data = request.POST, instance=request.user)

		#if form is valid, do something
		if i_form.is_valid():
			i_form.save()
			comment = i_form.cleaned_data.get('upload_description')
			comment_profile = ImageComment.objects.create(upload_description = comment, user = request.user)
			result = "perfect"
			message = "Your profile has been updated"
			context = {"result": result, "message": message,}
		else:
			message = FormErrors(i_form, i_form)
			context = {"result": result, "message": message}
	...
	return render(request, 'users/images.html', context)
...
def dropzone_image(request):
	...
	if request.method == "POST":
		...
		#get last comment object
		comment = ImageComment.objects.filter(user = user).order_by('-timestamp')[0]
		img = UserImage.objects.create(image = image, user = user, upload_description = comment.upload_description)
